...
async function obterAgendamentos() {
  const resposta = await fetch("https://SEU-DOMINIO-RAILWAY/agendamentos");
  const dados = await resposta.json();
  return dados;
}

async function salvarAgendamento(agendamento) {
  await fetch("https://SEU-DOMINIO-RAILWAY/agendamentos", {
    method: "POST",
    headers: {
...
async function finalizarAgendamento(id) {
  await fetch(`https://SEU-DOMINIO/agendamentos/${id}`, {
    method: "PUT",
    headers: {
      "Content-Type": "application/json"
    },
